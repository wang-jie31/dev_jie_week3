...
async function get<T>(path: string, revalidate?: number): Promise<T> {
  const res = await fetch(`${API_BASE}${path}`, {
    // ISR：若调用方传入 revalidate 秒数，则走 Next 数据缓存
    ...(revalidate
      ? { next: { revalidate } }
...
export const api = {
  home: (r?: number) => get<HomeData>("/api/v1/home", r ?? 60),
  cases: (params: Record<string, string | number | undefined>, r?: number) => {
    ...
    const q = qs.toString();
    return get<Paginated<CaseListItem>>(`/api/v1/cases${q ? `?${q}` : ""}`, r ?? 30);
  },
  caseDetail: (slug: string, r?: number) =>
    get<CaseDetail>(`/api/v1/cases/${slug}`, r ?? 30),
  ...
  packages: (params: Record<string, string | number | undefined>, r?: number) => {
    ...
    const q = qs.toString();
    return get<Paginated<PackageListItem>>(`/api/v1/packages${q ? `?${q}` : ""}`, r ?? 30);
  },
  packageDetail: (slug: string, r?: number) =>
    get<PackageDetail>(`/api/v1/packages/${slug}`, r ?? 30),
  news: (params: Record<string, string | number | undefined>, r?: number) => {
    ...
    const q = qs.toString();
    return get<Paginated<NewsItem>>(`/api/v1/news${q ? `?${q}` : ""}`, r ?? 60);
  },
  newsDetail: (slug: string, r?: number) => get<NewsItem>(`/api/v1/news/${slug}`, r ?? 60),
  team: (r?: number) => get<TeamMember[]>("/api/v1/team", r ?? 60),
  careers: (params: Record<string, string | undefined>, r?: number) => {
    ...
    const q = qs.toString();
    return get<CareerItem[]>(`/api/v1/careers${q ? `?${q}` : ""}`, r ?? 60);
  },
  site: (r?: number) => get<SiteConfig>("/api/v1/site", r ?? 60),
  contactInfo: (r?: number) => get<Record<string, unknown>>("/api/v1/about/contact-info", r ?? 60),
};
